...
(function() {
    ...
    var Modss = {
        init: function() {
                ...
                setTimeout(function() {
                    $('.extensions__item-author', Lampa.Extensions.render()).map(function(i, e) {
                        if (e.textContent == '@modss_group')
                            $(e).html('💎').append('<span class="extensions__item-premium">VIP buy at @modssmy_bot</span>');
                    });
                }, 500);
            });
            ...
            Lampa.Controller.listener.follow('toggle', function(e) {
                if (e.name == 'select' && !vip) {
                    setTimeout(function() {
                        if ($('.selectbox .scroll__body div:eq(0)').html() && $('.selectbox .scroll__body div:eq(0)').html().indexOf('.land') >= 0)
        ...
        radio: function() {
            ...
            if (Lampa.Storage.get('mods_radio'))
                ...
                $('body').find('[data-action="Radio_n"]').remove();
        ...
        tv_modss: function() {
            ...
            if (Lampa.Storage.get('mods_tv'))
                ...
                $('body').find('[data-action="modss_tv"]').remove();
        ...
        showModssVip: function() {
            ...
            Lampa.Modal.render().find('.cub-premium__descr:eq(1)').after('<div class="cub-premium__descr"><h3>👇 Кнопка для просмотра 👇</h3><img src="https://lampa.stream/but_modss.png"></div>');
        ...
        online: function(back) {
            ...
            var params = {
                url: '',
                title: Lampa.Lang.translate('modss_title_online') + '  💎 16 дней',
            ...
            var button = "<div style='position:relative' data-subtitle='modss_v".concat(manifest.version, " (22 Balansers, 19 in VIP)' class='full-start__button selector view--modss_online'>" + ico + "<span>" + this.title + "</span><div style='position: absolute;left: 1em;bottom: 1em;background-color: rgb(255, 255, 255);color: rgb(0, 0, 0);padding: 0.1em 0.2em;font-size: 0.6em;border-radius: 0.5em;font-weight: 900;text-transform: uppercase;'>VIP</div></div>");
            ...
            if (!back && Lampa.Storage.field('mods_onl')) {
                ...
                btn.unbind('hover:enter hover.click').on('hover:enter hover.click', function() {
                    inf();
                    Lampa.Activity.active().activity.render().find('.view--modss_online').html(Lampa.Lang.translate(ico + '<span>' + _this.title + '</span>') + "<div style='position: absolute;left: 1em;bottom: 1em;background-color: rgb(255, 255, 255);color: rgb(0, 0, 0);padding: 0.1em 0.2em;font-size: 0.6em;border-radius: 0.5em;font-weight: 900;text-transform: uppercase;'>VIP</div>");
                    if (_this.is_continue && Lampa.Storage.field('online_continued'))
                        shows(_this.last_s);
                    else
                        openOnline();
                });
        ...
        preload: function(e) {
            ...
            load.createSource(true).then(function(ok) {
                ...
                Lampa.Noty.show(e && (e.error ? 'Ошибка: ' + e.error : 'Ничего не найдено'));
                Lampa.Activity.active().activity.render().find('.view--modss_online').css('opacity', '0.3').html(Lampa.Lang.translate(ico + '<span>' + _this.title + '</span>') + "<div style='position: absolute;left: 1em;bottom: 1em;background-color: rgb(255, 255, 255);color: rgb(0, 0, 0);padding: 0.1em 0.2em;font-size: 0.6em;border-radius: 0.5em;font-weight: 900;text-transform: uppercase;'>VIP</div>");
                timer.stop();
        ...
        collections: function() {
            ...
            if (Lampa.Storage.get('mods_collection'))
                $('body').find('.menu .menu__list li:eq(3)').after(menu_item)
            else
                $('body').find('[data-action="collection"]').remove();
        ...
        last_view: function(data) {
            ...
            if (viewed) {
                ...
                $('body').find('.timeline,.card--last_view').remove();
            if ($('body').find('.online').length == 0)
                $('.card--new_ser,.card--viewed').remove();
        },
        serialInfo: function(card) {
            if (Lampa.Storage.field('mods_serial_info') && card.source == 'tmdb' && card.seasons && card.last_episode_to_air) {
                ...
                if (card.next_episode_to_air) {
                    ...
                    new_ser = last_seria_inseason + ' #{season_ended}';
                ...
                if (!$('.card--new_seria', Lampa.Activity.active().activity.render()).length) {
                    if (window.innerWidth > 585)
                        ...
                        if ($('.card--new_seria', Lampa.Activity.active().activity.render()).length)
                            $('.full-start__tags', Lampa.Activity.active().activity.render()).append('<div class="full-start__tag card--new_seria"><img src="./img/icons/menu/movie.svg" /> <div>' + Lampa.Lang.translate(new_ser) + '</div></div>');
                        else
                            $('.full-start-new__details', Lampa.Activity.active().activity.render()).append('<span class="full-start-new__split">●</span><div class="card--new_seria"><div>' + Lampa.Lang.translate(new_ser) + '</div></div>');
                    }
                }
            }
        },
        rating_kp_imdb: function(card) {
            return new Promise(function(resolve, reject) {
                ...
                Pub.network.silent(API + 'KPrating', function(json) {
                    ...
                    if ((!vip && logged == 'false' && leftVipD !== json.data.leftDays && auth && json.data.vip) || (vip && logged == true && leftVipD !== json.data.leftDays && auth == 'false' && !json.data.vip))
        ...
        auth: function(kp) {
            ...
            .then(function(start) {
                ...
                if (!start.block_ip)
                    ping_auth = setInterval(authFn, start.interval);
            })
        ...
        balansers: function() {
            var balansers = {
                ...
                "kinofit": "KinoFit 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>"
            };
            if (Lampa.Storage.get('pro_pub', false))
                balansers = Object.assign({
                    "pub": "Pub"
                }, balansers);
            return balansers;
        },
        check: function(name, call) {
            ...
            var check = function check(ok) {
                Pub.network["native"](Protocol() + u, function(t) {
                    if (name && !call)
                        item2.removeClass('active error wait').addClass('active');
                    if (call) {
                        if (name && !Modss.jack[name].check)
                            Modss.jack[name].check = true;
                        if (name && !Modss.jack[name].ok)
                            Modss.jack[name].ok = true;
                        call(true);
                    }
                }, function(a, c) {
                    ...
                    if (name && !call)
                        item2.removeClass('active error wait').addClass('error');
                    if (call) {
                        if (ok && name && !Modss.jack[name].check)
                            Modss.jack[name].check = true;
                        if (ok && name && !Modss.jack[name].ok)
                            Modss.jack[name].ok = false;
                        call(false);
                    }
                });
            };
            if (name && !call)
                check();
            else if (call && name && !Modss.jack[name].check)
                check(true);
            else {
                if (name && Modss.jack[name].ok)
                    if (call)
                ...
                if (Boolean(Modss.jack[Lampa.Storage.get('jackett_url2')]))
                    item.removeClass('wait').addClass(Modss.jack[Lampa.Storage.get('jackett_url2')].ok ? 'active' : 'error');
            }
        },
        jack: {
            jacred_xyz: {
        ...
        showStatus: function(ch) {
            ...
            if (status.login) {
                ...
                if (status.is_pro || status.is_pro_plus)
                    info = foto + ' - <b>' + (status.is_pro ? 'PRO' : 'PRO_PLUS') + '</b> ' + Lampa.Lang.translate('filter_rating_to') + ' - ' + status.pro_date + '</span>';
                else
                    info = foto + ' - <b>NO PRO</b> - MAX 720p</span>';
            }
            if (ch)
                $('.settings-param__descr', ch).html(info);
            else
                $('.settings-param__descr:eq(0)').html(info);
        },
        checkPro: function(token, call, err) {
            if (!token && typeof call == 'function')
                call({});
            this.network.clear();
    ...
    var ForkTV = {
        ...
        init: function() {
            if (Lampa.Storage.get('mods_fork'))
                ...
                if (json.channels.length === 1) {
                    ...
                    if (itm) {
                        if (itm.text().indexOf('код') == -1 || itm.text().indexOf('code') == -1)
                            $('.settings-param__descr', itm).html('<img width="30em" src="./img/logo-icon.svg"> <b style="vertical-align: middle;font-size:1.4em;color:#FF8C00">' + Lampa.Lang.translate('account_authorized') + '</b>');
                        if (itm.find('.settings-param__name').text().indexOf('раздел') > -1 || itm.find('.settings-param__name').text().indexOf('Sections') > -1)
    ...
    function videocdn(component, _object) {
        ...
        this.searchs = function(_object) {
            ...
            var imdb_params = object.movie.imdb_id ? Lampa.Utils.addUrlComponent(url, 'imdb_id=' + encodeURIComponent(object.movie.imdb_id)) : '';
            var kp_params = +object.movie.kinopoisk_id || +object.movie.kinopoisk_ID ? Lampa.Utils.addUrlComponent(url, 'kinopoisk_id=' + encodeURIComponent(+object.movie.kinopoisk_id || +object.movie.kinopoisk_ID)) : '';
            ...
            network.silent(imdb_params || kp_params, function(json) {
            ...
            if (fs) {
                    ...
                    for (var a in files[i].json) {
                        ...
                        if (elem.folder) {
                            ...
                            if (season_num == null && elem.comment) {
                                ...
                                if (str_s)
                                    season_num = parseInt(str_s[1]);
                            }

                            if (season_num == null)
                                season_num = season_count;
                            elem.season_num = season_num;
                            if (extract.season_num.indexOf(season_num) == -1)
                                extract.season_num.push(season_num);
                            ...
                            for (var f in elem.folder) {
                                ...
                                if (media.comment) {
                                    ...
                                    if (str_v)
                                        alt_voice = str_v[1].trim();
                                }

                                if (alt_voice && !voice_name)
                            ...
                            if (extract.season_num.indexOf(_season_num) == -1)
                                extract.season_num.push(_season_num);
                            ...
                            if (_episode_num == null && elem.comment) {
                                ...
                                if (_str_e)
                                    _episode_num = parseInt(_str_e[1]);
                            }

                            if (_episode_num == null)
                                _episode_num = root_count;
                            var _alt_voice = '';
                            ...
                            if (elem.comment) {
                                ...
                                if (_str_v)
                                    _alt_voice = _str_v[1].trim();
                            }

                            if (_alt_voice && !voice_name)
                                voice_name = _alt_voice;
                ...
                if (extract.voice.length > 1 || extract.season_num.length) {
                    extract.voice.forEach(function(v) {
                        if (!v.name)
                            v.name = v.id == 0 ? 'Перевод' : v.id == 7 ? 'Оригинал' : v.id + '';
                    });
                }
        ...
        function filter() {
            ...
            if (choice.voice_name) {
                ...
                if (choice.voice_id) {
                    ...
                    if (voice)
                        inx = filter_items.voice_info.indexOf(voice);
                }

                if (inx == -1)
                    inx = filter_items.voice.indexOf(choice.voice_name);
                if (inx == -1)
                    choice.voice = 0;
            ...
            component.draw(items, {
                ...
                onEnter: function onEnter(item, html) {
                    ...
                    if (extra.file) {
                        ...
                        Lampa.Noty.show(Lampa.Lang.translate(get_links_wait ? 'modss_waitlink' : 'modss_nolink'));
                },
    ...
    function rezka(component, _object) {
        ...
        function getChoiceVoice() {
            ...
            if (choice.voice_token) {
                extract.voice.forEach(function(voice) {
                    if (voice.token === choice.voice_token)
                        res = voice;
                });
            }
        ...
        function getFirstTranlate(id, call) {
            ...
            network["native"](embed + 'embed/' + id, function(str) {
                ...
                if (extract.voice.length)
                    call(getChoiceVoice().token);
                else
                    component.doesNotAnswer(object.movie.title);
            }, function(a, c) {
        ...
        function filter() {
            ...
            if (!extract.season[choice.season])
                choice.season = 0;
            else
        ...
        function getStream(element, call, error) {
            ...
            if (element.voice.d)
                url += '&d=' + encodeURIComponent(element.voice.d);
    ...
    function cdnmovies(component, _object) {
        ...
        function getStream(element, call, error) {
            ...
            if (false)
                url = url.replace(/(\.mp4):hls:manifest\.m3u8$/i, '$1');
                ...
                s.folder.forEach(function(e) {
                    if (e.folder) {
                        e.folder.forEach(function(v) {
                            ...
                            if (filter_items.voice.indexOf(voice) == -1)
            ...
            if (extract[choice.season] && extract[choice.season].folder) {
                ...
                extract[choice.season].folder.forEach(function(f) {
                    f.folder.forEach(function(t) {
                        if (filter_items.voice.indexOf(t.title) == -1)
                            filter_items.voice.push(t.title);
                    });
                });
            }
            ...
            component.draw(items, {
                ...
                onEnter: function onEnter(item, html) {
                    ...
                    getStream(item, function(stream) {
                        ...
                        if (item.subtitles && Lampa.Player.subtitles)
                            Lampa.Player.subtitles(item.subtitles);
                        item.mark();
                    }, function(data) {
                        item.loading = false;
    ...
    function component(object) {
        ...
        this.initialize = function() {
            ...
            filter.onSelect = function(type, a, b) {
                if (a.bal) {
                    ...
                    if (a.reset)
                        ...
                        var url;
                        if (a.stype == 'voice') {
                            var season = filter_find.season[choice.season] && parseInt(filter_find.season[choice.season].split(' ')[1]) || false;
                            if (!filter_find[a.stype][b.index])
                                return reset();
                        ...
                        if (a.stype == 'server' || a.stype == 'type') {
                            if (a.stype == 'server')
                                Lampa.Storage.set('pub_server', b.index);
                            if (a.stype == 'type')
                                Lampa.Storage.set('pub_type_striming', b.index);
                            _this.initialize();
                        } else if (url) {
                ...
                if (object.movie.number_of_seasons || balanser == 'pub' || balanser == 'bazon')
            ...
            if (filter.addButtonBack && !$('.filter--back').length)
            ...
            this.createSource().then(function(wait) {
                ...
                if (['seasonvar', 'kinotochka', 'hdrezka', 'pub', 'kinokrad', 'kinobase', 'filmix', 'qiwi', 'videocdn', 'eneida'].indexOf((balanser.indexOf('<') == -1 ? balanser.toLowerCase() : balanser.split('<')[0].toLowerCase())) == -1) {
        ...
        this.requestParams = function(url) {
            ...
            if (Filmix.token)
                query.push('token=' + Filmix.token);
            query.push('cub_id=b2theXNoaXRAZ21haWwuY29t');
            query.push('uid=0c01865cce6862fe6359edc6b_64841958');
            if (cards) {
                var notices = Lampa.Storage.get('account_notice', []).filter(function(n) {
        ...
        this.getLastChoiceBalanser = function() {
            ...
            if (priority_balanser == undefined)
                priority_balanser = Modss.balansPrf;
        ...
        this.startSource = function(json) {
            return new Promise(function(resolve, reject) {
                ...
                if (filter_sources.length) {
                    ...
                    if (!sources[balanser].show && !object.modss_custom_select)
                        balanser = filter_sources[0];
                ...
                var downloadFromUrl = function(downloadUrl) {
                    ...
                    xhr.onerror = function(e) {
                        if (xhr.status === 0 && !retryAttempt) {
                            ...
                            if (url)
                                downloadFromUrl(url.replace('https://www.opensubtitles.org/download/s/', 'https://www.opensubtitles.org/ru/download/s/').replace('https://www.opensubtitles.org/ru/subtitleserve/sub/', 'https://dl.opensubtitles.org/ru/download/sub/'));
                            else
                                reject(new Error('Ошибка загрузки архива URL не должен быть пустой'));
                        } else
                            reject(new Error('Ошибка загрузки архива'));
                    };
                    xhr.send();
                };
                downloadFromUrl(url);
                    ...
                    if (json.serial) {
                        ...
                        for (var key in json.data) {
                            if (json.data.hasOwnProperty(key)) {
                                ...
                                .then(function(data) {
                                    console.log('Modss', 'loadedInf', (imdb || id), key + ' загружен успешно:', data);
                                })
                                .catch(function(error) {
                                    console.error('Modss', 'loadedInf', (imdb || id), title, ' - Ошибка при загрузке языка - ' + key, error);
                                });
                            }
                        }
                    }
        ...
        this.getFileUrl = function(file, call) {
            this.loadedInf(object.movie.id, object.movie.imdb_id, encodeURIComponent(object.movie.original_title || object.movie.original_name), ((object.movie.release_date || object.movie.first_air_date || '0000') + '').slice(0, 4), file.season, file.episode);

            var _this = this;
            if (balanser == 'hdr' && navigator.userAgent.toLowerCase().indexOf('android') >= 0 && !Lampa.Platform.is('android'))
                return Lampa.Platform.install('apk');

            if (file.method == 'play') {
                if (false /*file.url && ['hdrezka'].indexOf(balanser) >= 0  && 'LT' == 'UA'*/
                ) {
                    Lampa.Loading.start(function() {
                        Lampa.Loading.stop();
                        Lampa.Controller.toggle('content');
                    });
                    _this.checkHashR(file).then(function(data) {
                        Lampa.Loading.stop();
                        call(data, {});
                    }, function(err) {
                        Lampa.Loading.stop();
                ...
                if (Lampa.Storage.field('player') !== 'inner' && Lampa.Platform.is('apple') && file.stream) {
                    ...
                    network["native"](file.url, function(json) {
                        if (json.url && ['kodik'].indexOf(balanser) >= 0 && ['UA'].indexOf('LT') >= 0) {
                ...
                onEnter: function onEnter(item, html) {
                    _this4.getFileUrl(item, function(json, json_call) {
                        if (json && json.url) {
                            ...
                            if (!first.subtitles)
                                first.subtitles = item.subtitles;
                                                ...
                                                cell.url = function(call) {
                                                    _this4.getFileUrl(elem, function(stream, stream_json) {
                                                        if (stream.url) {
                                                            ...
                                                            if (elem.subtitles && elem.subtitles.length && elem.method == 'call' && json.subtitles)
                                                                cell.subtitles = stream.subtitles.concat(elem.subtitles);
                                                            if (!cell.subtitles)
                                                                cell.subtitles = elem.subtitles;
                                                            cell.translate = {
                            ...
                            } else //Lampa.Player.playlist(playlist);
                            {
                                playlist.push(first);
                            }
                            if (playlist.length > 1)
                            ...
                            Lampa.Noty.show(json && json.vip ? json.vip.title + '<br>' + json.vip.msg : Lampa.Lang.translate('modss_nolink'));
                    }, true);
                },
                onContextMenu: function onContextMenu(item, html, data, call) {
        ...
        this.parse = function(json) {
            try {
                ...
                if (extract.similars) {
                    ...
                    if (extract && extract.server)
                        filter_find.server = extract.server;
                    if (extract && extract.type)
                        filter_find.type = extract.type;
                    ...
                    if (extract.voice && typeof extract.voice[0] == 'object')
                        filter_find.voice = extract.voice;
                    if (season) {
                    ...
                    } else if (!Lampa.Arrays.getKeys(items).length)
                        this.empty();
                }
            } catch (e) {
                    ...
                    window.addEventListener('message', function(event) {
                        ...
                        if (event.origin === proxyOrigin && data && data.message === 'proxyResponse' && data.message_id) {
                            ...
                            if (call) {
                                ...
                                if (data.status === 200) {
                                    if (call.success)
                                        call.success(data.responseText);
                                } else {
                                    if (call.fail)
                                        call.fail({
                                            status: data.status,
        ...
        this.fixLink = function(link, proxy, referrer) {
            if (link) {
                if (!referrer || link.indexOf('://') !== -1)
                    return proxy + link;
                var url = new URL(referrer);
                if (startsWith(link, '//'))
                    return proxy + url.protocol + link;
                if (startsWith(link, '/'))
                    return proxy + url.origin + link;
                if (startsWith(link, '?'))
                    return proxy + url.origin + url.pathname + link;
                if (startsWith(link, '#'))
                    return proxy + url.origin + url.pathname + url.search + link;
        ...
        this.similars = function(json) {
            ...
            json.forEach(function(elem) {
                ...
                if (orig && orig !== name)
                    info.push(orig);
                if (elem.type)
                    info.push(elem.type == 'tv-series' || elem.type == 'serial' || elem.type == 'MINI_SERIES' ? 'Cериал' :
                    elem.type == 'TV_SHOW' ? 'Тв-Шоу' :
                    elem.type == ('movie' || 'film' || 'FILM') ? 'Фильм' : elem.type);
                if (count_s)
                    info.push((count_s && ' - ' + count_s + ' из них ' + count_eps));
                if (!count_s && count_eps)
                    info.push(count_eps);
                if (transl)
                    info.push(transl);
                if (elem.rating && elem.rating !== 'null' && elem.filmId)
                    info.push(Lampa.Template.get('modss_online_rate', {
                        rate: elem.rating
                    }, true));
                if (elem.quality && elem.quality.length)
                    info.push(elem.quality);
        ...
        this.filter = function(filter_items, choice) {
            ...
            var countVoic = function countVoic(data, season) {
                ...
                Lampa.Arrays.getKeys(data).forEach(function(dub) {
                    if (data[dub].hasOwnProperty(season))
                        count++;
                });
                return count;
            };
            ...
            var add = function add(type, title) {
                ...
                /*
                        var filteredItems = type === 'voice' ? subitems.filter(function (el) {
                          return el.find && el;
                        }) : subitems;
                */
                select.push({
        ...
        this.draw = function(items) {
            ...
            this.getEpisodes((object.movie.source == 'pub' || balanser == 'pub') || (object.movie.original_language !== 'ja' || object.movie.number_of_seasons >= choice.seasons) ? items[0].season : 1, function(episodes) {
                ...
                items.forEach(function(element, index) {
                    ...
                    if (element.sizeB)
                        element.quality = Lampa.Utils.calcBitrate(element.sizeB, episode ? episode.runtime : object.movie.runtime) + Lampa.Lang.translate('torrent_item_mb') + ' - ' + element.quality;
                    ...
                    if (episode) {
                        ...
                        if (episode.air_date && fully && !element.info.includes(Lampa.Utils.parseTime(episode.air_date).full))
                    ...
                    if (!serial && object.movie.tagline && element.info.length < 30 && !element.info.includes(object.movie.tagline))
                    ...
                    if (serial && episode_num && filter_find.season.length)
                        image.append('<div class="online_modss__episode-number-season">S' + (element.season || episode && (episode.snumber || episode.season_number) || 0) + ':E' + (element.episode || episode && (episode.number || episode.episode_number) || 0) + '</div>');
                    ...
                    if (element.subtitles)
                        html.find('.online_modss__img').append('<div class="online_modss__subtitle">' + Lampa.Template.get('icon_subs', {}, true) + '</div>');
        ...
        this.new_seria = function() {
            if (object.movie.number_of_seasons) {
                setTimeout(function() {
                    ...
                    if ($('body').find('.online').length !== 0) {
                        if ($('body').find('.online:last-child .torrent-item__viewed').length == 1 || $('body').find('.online:last-child .time-line.hide').length == 0)
                            $('body').find('.full-start__poster').append("<div class='card--viewed' style='right: -0.6em;position: absolute;background: #168FDF;color: #fff;top: 0.8em;padding: 0.4em 0.4em;font-size: 1.2em;-webkit-border-radius: 0.3em;-moz-border-radius: 0.3em;border-radius: 0.3em;'>" + Lampa.Lang.translate('online_viewed') + "</div>");
                        else
                            $('body').find('.full-start__poster').append("<div class='card--new_ser' style='right: -0.6em;position: absolute;background: #168FDF;color: #fff;top: 0.8em;padding: 0.4em 0.4em;font-size: 1.2em;-webkit-border-radius: 0.3em;-moz-border-radius: 0.3em;border-radius: 0.3em;'>" + Lampa.Lang.translate('season_new') + " " + Lampa.Lang.translate('torrent_serial_episode') + "</div>");
                    }
        ...
        this.updateWatched = function() {
            ...
            if (watched) {
                var line = [];
                if (watched.balanser_name)
                    line.push(watched.balanser_name);
            ...
            balanser_timer = setInterval(function() {
                ...
                if (tic == 0) {
                    ...
                    if (!next)
                        next = keys[0];
                    balanser = next;
                    if (Lampa.Activity.active().activity == _this8.activity)
                        _this8.changeBalanser(balanser);
                }
            }, 1000);
        };
            ...
            Lampa.Controller.add('content', {
                ...
                right: function right() {
                    if (Navigator.canmove('right'))
                        ...
                        filter.show(Lampa.Lang.translate('modss_balanser'), 'sort');
                },
                left: function left() {
                    var poster = files.render().find('.explorer-card__head-img');
                    if (poster.hasClass('focus'))
                        Lampa.Controller.toggle('menu');
                    else if (Navigator.canmove('left'))
                        Navigator.move('left');
                    else
                        Navigator.focus(poster[0]);
                },
                        ...
                        if (element.title.match('Описание|Трейлер') == null) {
                            if (element.template && element.template.indexOf('film.') > -1 || element.logo_30x30 && element.logo_30x30.match('mediafil') || element.logo_30x30 && element.logo_30x30.match('folder') && element.playlist_url && element.playlist_url.indexOf('torrstream?magnet') > -1) {
                                ...
                                if (element.logo_30x30 && element.logo_30x30.match(/folder|mediafil/) && stream && stream.match(/torrstream\?magnet|getstream|kinomix/)) {
                                    ...
                                    if (sed_per || vid || sizes || vo)
                                        element.info = (sed_per ? '<b style="color:green">&#8679;' + parseInt(sed_per[1]) + '</b><b style="color:red">&#8659;' + parseInt(sed_per[2]) + '</b> - ' : '') + (vo ? vo[1] + ' / ' : '') + (sizes && ' <b>' + sizes + '</b><br><hr>' || '') + (vid ? vid[0].replace(/Аудио|Звук/, ' | Аудио') : '');
                                }
                                ...
                                if (!element.search_on) {
                                    ...
                                    var peer = info && info.split(/<br[^>]*>|<\/div>/).find(function(itm) {
                                        if (itm.match(/Качают|Скачивают|Leechers/))
                                            return itm;
                                    });
                                    var seed = info && info.split(/<br[^>]*>|<\/div>/).find(function(itm) {
                                        if (itm.match('Раздают|Seeders'))
                                            return itm;
                                    });
                                }
                                ...
                                if (!data.landscape && !data.details && ((/iPhone|android/i.test(navigator.userAgent) || Lampa.Platform.is('android'))))
                                    card.addClass('mobile');
                                if (/iPhone|x11|nt/i.test(navigator.userAgent) && !Lampa.Platform.is('android'))
                                    card.addClass('pc');
                                if (/Mozilla/i.test(navigator.userAgent) && !/Android/i.test(navigator.userAgent) || Lampa.Platform.tv())
                                    card.addClass('tv');
                                if (data.details && !data.details.images && stream && stream.match(/subcategory|submenu|page=|year=|list\?direc|genre|list\?actor|country/g) !== null)
                                    card.addClass('mobiles');
                                if (element.description && element.description.indexOf('linear-gradientto') > -1 || data.landscape || data.next_page_url && data.next_page_url.indexOf('girl') > -1)
                                    card.addClass('nuam');
                                if (data.next_page_url && data.next_page_url.indexOf('girl') > -1 && stream.indexOf('vporn/list?cat'))
                                    card.addClass('card--category').removeClass('card--collection');
                                if (element.logo_30x30 && element.logo_30x30.match(/country|genre|filter|mediafolder/g) != null)
                                    card.addClass('hdgo');
                                if (element.logo_30x30 && element.logo_30x30.match(/\/folder\./g) && stream.match(/stream|magnet|view\?|view=|\/details/g))
                                    card.addClass('mobile card--category').removeClass('card--collection');
                                if (element.logo_30x30 && element.logo_30x30.indexOf('/folder.') > -1 && stream.match(/view=/g))
                                ...
                                if (element.logo_30x30 && element.logo_30x30.match(/mediafolder/g))
                                    card.addClass('card--category').removeClass('card--collection');
                                if (bg_img.background_image && bg_img.background_image.indexOf('18') > -1 && ((data.next_page_url && data.next_page_url.indexOf('girl') > -1) && stream.match(/pornst|models/g) !== null))
                                    card.addClass('card--category').removeClass('nuam hdgo mobile card--collection');
                                if (image && image.indexOf('film.svg') > -1)
                                    card.addClass('card--collection nuam');
                                if (bg_img.background_image && bg_img.background_image.indexOf('18') > -1 && stream.match(/view\?|hdporn|channel=/g))
                                    card.addClass('card--collection').removeClass('nuam hdgo mobile card--category');
                                if (object.title.match(/Торренты|ForkTV|18\+/g))
                                    card.addClass('home');
                                if (element.logo_30x30 && element.logo_30x30.match(/country|genre|filter/g))
                                    card.addClass('sort');
                                if ((stream && stream.match(/filmix\?subcategory|rutor/) || element.submenu && element.submenu[0] && element.submenu[0].playlist_url && element.submenu[0].playlist_url.indexOf('rutor') > -1) && element.logo_30x30 && element.logo_30x30.match(/filter/g))
                                    card.addClass('two');
                                if (element.title == 'Поиск' && (stream && stream.match(/coldfilm/) || object.title == 'SerialHD'))
                                    card.addClass('searc');
                            ...
                            card.on('hover:focus hover:touch', function() {
                                if (this.className.indexOf('card--category') > -1) {
                                    if (Lampa.Helper)
                                        Lampa.Helper.show('online_file', 'Удерживайте клавишу (ОК) для просмотра описания', card);
                                ...
                                if (Math.ceil(items.indexOf(card) / 7) >= maxrow)
                                    if (data.next_page_url)
                                        _this3.next(data.next_page_url);
                                    ...
                                    if (element.event || (stream && stream.match(/youtube|stream\?|mp4|mkv|m3u8/i))) {
                                        ...
                                        if (element.title == '18+' && Lampa.Storage.get('mods_password')) {
                                            Lampa.Input.edit({
                                                ...
                                                if (Lampa.Storage.field('mods_password') == t)
                                                    goto();
                                                else {
    ...
    function modss_tv(object) {
        ...
        this.saveCats = function() {
            ...
            filter_sources.forEach(function(i, k) {
                if (i.title.split(' - ')[0] == servOpt)
                    filter_sources[k] = server;
                if (i.title.split(' - ')[0] == lasts)
                    filter_sources[k].selected = true;
            });
            ...
            filter.render().find('.filter--search').unbind('hover:enter').on('hover:enter', function(e) {
                Lampa.Input.edit({
                    ...
                    Lampa.Arrays.getValues(results).forEach(function(i) {
                        i.find(function(l) {
                            if (l.name && encodeURIComponent(l.name.trim().toLowerCase()).match(encodeURIComponent(t.trim().toLowerCase())))
                                data.push(l);
                        });
                    });
                    ...
                    if (data && data.length) {
                        ...
                        Lampa.Noty.show(Lampa.Lang.translate('online_query_start') + ' (' + t + ') ' + Lampa.Lang.translate('online_query_end'));
                    Lampa.Controller.toggle('content')
                });
            });
        ...
        this.updateFilter = function(data) {
            ...
            if (Lampa.Storage.field('mods_tv_style') == 'vert')
                filter.set('filter', filter_sources);
            ...
            if (data && (data.length || Lampa.Arrays.getKeys(data).length)) {
                ...
                if (object.url.indexOf('Fork') == -1 && Lampa.Storage.field('mods_tv_style') == 'vert') {
                    html.find('.scroll:eq(1)').css({
                        'position': 'relative',
                        'min-height': '30em'
                    }).parent().append('<div class="epg" style="position:absolute;width:100%;top:18em;left:0;"><div class="program"></div></div>');
        ...
        this.builder = function(data) {
            ...
            if (data && (data.length || Lampa.Arrays.getKeys(data).length)) {
                !searched && data.forEach(function(element) {
                    if (element.items)
                        results[element.group || 'IPTV'] = element.items;
                    else if (element.servsmodstv_url && !results[servOpt])
                        results[servOpt] = [element];
                    else if (element.servsmodstv_url && results[servOpt])
                        results[servOpt].push(element);
                    else if (results[element.group || 'IPTV'])
                        results[element.group || 'IPTV'].push(element);
                    else
                        results[element.group || 'IPTV'] = [element];
                });
                ...
                if (searched && data.length)
                    ...
                    if (Lampa.Arrays.getKeys(results).length == 1)
                        _this.append_v(results['IPTV']);
                    else {
                        _this.saveCats();
                        for (var key in results) {
                            if (filter_sources.find(function(a) {
        ...
        this.append_v = function(data) {
            ...
            data.forEach(function(element) {
                ...
                card.on('hover:focus hover:touch', function(e, is_mouse) {
                    ...
                    if (element.title || data[0].results || object.url.indexOf('pub/tv') > -1)
                        return;
                    var enabled = Lampa.Controller.enabled().name;
                    var menu = [];
                    if (object.url.indexOf('Fork') > -1 || Lampa.Storage.field('mods_tv_style') == 'vert')
                    ...
                    if (element.results) {
                        ...
                        if (element.servsmodstv_url) {
                            ...
                            if (!element.video)
                                return Lampa.Noty.show(Lampa.Lang.translate('modss_nolink'));
                            if (element.video.match(/png|jpeg|jpg|bmp/))
                                return;
                            var playlist = [];
        ...
        this.creates = function(data) {
            ...
            this.append = function(element) {
                ...
                if (data.title.indexOf('Избранные') > -1 || in_favorite)
                    _this1.addicon('like', item$1.render());

                if (element.servcdn)
                    item$1.render().find('.card__icons').append('<div class="card__type" style="display:none;top:-.3em!important;left:.7em!important">' + element.servcdn + '</div>');
                item$1.render().on('hover:focus hover:touch', function() {
                    $('.card__type').hide();
                    ...
                    if (!element.video)
                        return Lampa.Noty.show(Lampa.Lang.translate('modss_nolink'));
                    function play() {
                        if ((element.video || element.tvmedia).match(/png|jpeg|jpg|bmp/))
                            return;
                        var video = {
        ...
        this.epg = function(name, id) {
            ...
            api.parseEpg(name, id).then(function(json) {
                ...
                if (Lampa.Storage.field('mods_tv_style') == 'cats')
                    bod.find('.selector:eq(0)').before(Lampa.Lang.translate('<div style="font-size:1.8em;font-weight: 700;margin-left:.5em">#{iptv_now} ' + name + '</div>'));
                ...
                if (now == 'player') {
                    ...
                    if ($('.notice:eq(1) .notice__time', epg).html()) {
                        ...
                        if (Lampa.Player.render().find('#efir').length == 0)
                            Lampa.Player.render().find('.player-info__values').after('<div id="efir" style="margin-top:.5em"></div>');
        ...
        this.favorite = function(data, card) {
            ...
            if (in_favorite)
                this.addicon('like', card);
            else
                card.find('.card__icons-inner').empty();
    ...
    function collection(object) {
        ...
        this.create = function() {
            ...
            if (object.sourc == 'my_coll') {
                ...
                if (object.card && isNaN(object.id))
                    url = object.id;
                else if (object.sourc == 'pub') {
                    if (object.search)
                        url = object.url + '?title=' + object.search + '&sort=views-&access_token=' + Pub.token;
                    else
                        url = object.url + '?sort=' + (object.sort ? object.sort : 'views-') + '&access_token=' + Pub.token;
                } else if (object.sourc == 'rezka')
                    url = object.url + '?filter=last';
                else
                    url = object.url;
                ...
                if ((object.page == 1 && object.card_cat) || object.cards || (!object.card && !Lampa.Storage.field('light_version') && object.card_cat)) {
                    ...
                    network.silent(cors + url, function(str) {
                        ...
                        _this.build(data);
                        if (object.card)
                            $('.head__title').append(' - ' + data.card.length);
                    }, function(a, c) {
        ...
        this.next = function(page) {
            ...
            if (object.sourc == 'pub' && object.sour !== 'rezka')
                url = object.url + '?page=' + object.page + '&sort=' + (object.sort ? object.sort : 'views-') + '&access_token=' + Pub.token;
            else if ((object.sourc == 'rezka' || object.sour == 'rezka') && object.data && object.data.page)
                url = object.data.page;
            else
                url = page.replace(/(\d+)\/\?filter/, object.page + '/?filter');
            eval(function(a, b, c) {
                if (a || c) {
                    while (a--)
                        b = b.replace(new RegExp(a, 'g'), c[a]);
        ...
        this.append = function(data, append) {
            ...
            datas.forEach(function(element) {
                ...
                if (object.category && (element.watch || element.quantity))
                    card.render().find('.card__view').append('<div style="background-color: rgba(0,0,0, 0.7);padding:.5em;position:absolute;border-radius:.3em;right:3;bottom:3">' + (element.watch || element.quantity) + '</div>');
                    ...
                    Lampa.Select.show({
                        ...
                        onSelect: function onSelect(a) {
                            if (a.where == 'clear') {
                                ...
                                if (Lampa.Storage.clean)
                                    Lampa.Storage.clean('my_colls');
                ...
                if (cache['id_' + element.id])
                    _this1.addicon('book', card.render());
                if (append)
        ...
        this.build = function(data) {
            ...
            if (data.card.length || Lampa.Arrays.getKeys(data.card).length) {
                ...
                if (object.sourc == 'pub' && object.category)
                    html.append(info);
                html.append(scroll.render());
        ...
        this.more = function(data) {
            ...
            var url;
            if (object.sourc == 'pub')
                url = object.url + '?page=' + data.page + '&sort=' + (object.sort ? object.sort : 'views-') + '&access_token=' + Pub.token;
            else
                url = data.page;
            network.silent(cors + url, function(result) {
        ...
        this.finds = function(element, find) {
            ...
            if (find.movie && find.movie.results.length)
                filtred(find.movie.results);
            if (find.tv && find.tv.results.length && !finded)
                filtred(find.tv.results);
            return finded;
        };
        this.start = function() {
            Lampa.Controller.add('content', {
    ...
    var PREFERRED_STREAMS = (useAAC) ? [// 320k MP3
    {
        urlRegex: /320\.pls$/,
        format: 'mp3'
    }, // 256k MP3
    ...
    function Info(station, Player) {
        ...
        this.create = function() {
            ...
            if (showAnalyzer !== 'hide' && !isInitialized)
                this.visualisation();
            this.update_info();
        };
        ...
        this.update_info = function() {
            ...
            if (_component._object.id == 2) {
                ...
                // get songs list for a channel from api
                _this.getSongs = function getSongs(channel) {
                    if (!channel || !channel.id || !channel.songUrl || !channel.songIdUrl || songId === "000")
                        return;
                    var noCache = new Date().getTime();
                    if (getNewSong) {
                        getNewSong = false;
                        Api.network.native(channel.songUrl + '?' + noCache, function(data) {
                            if (!data.uniqueid || data.uniqueid == '000')
                                return;
        ...
        this.visualisation = function() {
            ...
            if (showAnalyzer == 'line')
                Visualizer.visualizeBarGraph(played);
            else if (showAnalyzer == 'ball')
                Visualizer.visualizeGraphics(played);
        };
    ...
    function Player() {
        ...
        function prepare() {
            if (_audio && _audio.canPlayType('audio/vnd.apple.mpegurl'))
                load();
    ...
    function Radio_n(object) {
        ...
        this.create = function() {
            ...
            if (last_st)
                object = Object.assign({}, object, Api.stantion().find(function(s) {
                    if (object.url == '' || object.change || last_st.last)
                        return last_st.key == s.title;
                    if (!object.change)
                        return s.id == last_st.id;
                }));
            ...
            Api.list(object).then(function(result) {
                ...
                if (last_st.id == -1 && Favorites.get().length)
                    ...
                    if (!filtred.length)
                        filtred = _this6.data.stations;
                } else
                    filtred = _this6.data.stations && _this6.data.stations || _this6.data;
        ...
        this.buildChanel = function(result) {
            if (object.id == 2) {
                ...
                _this6.data = result.result && result.result || result;
        };
        ...
        this.buildStantion = function() {
            ...
            btn.on('hover:enter', function() {
                Lampa.Select.show({
                    ...
                    onSelect: function onSelect(a) {
                        if (!Lampa.Arrays.getKeys(cache[a.title]).length)
                            cache[a.title] = {};
        ...
        this.buildCatalog = function() {
            ...
            btn.on('hover:enter', function() {
                Lampa.Select.show({
                    ...
                    items: items.map(function(s) {
                        if (active && active.ghost)
                            s.selected = s.id == -2;
                        if (active && !active.ghost)
                            s.selected = s.id == active.id;
                        return s;
                    }),
                    onSelect: function onSelect(a) {
                        if (a.favorite) {
        ...
        this.next = function() {
            ...
            if (filtred.length > 100)
                ...
                filtred.forEach(_this6.append.bind(_this6));
            Lampa.Layer.visible(scroll.render(true));
        };
        ...
        this.append = function(station) {
            ...
            if (!station.id)
                station.id = (filtred.indexOf(station) + 1);
            ...
            if (station.listeners)
                item.find('.m-radio-item__listeners').append(station.listeners);
            else
                item.find('.m-radio-item__listeners').addClass('hide');
            ...
            item.on('hover:enter', function() {
                if (curPlayID !== station.id)
                    scroll.render().find('.play').removeClass('play pause');
                item.addClass('play');
                _this7.play(station);
        ...
        this.start = function() {
            ...
            Lampa.Controller.add('content', {
                ...
                left: function left() {
                    if ($('body').hasClass('ambience--enable') && !$('body').find('.m-radio-cover__img-container').hasClass('focus'))
                        return;
                    var cover = $('body').find('.m-radio-cover__img-container');
                    if (cover.hasClass('focus'))
                        Navigator.focus(last);
                    else if (Navigator.canmove('left'))
                        Navigator.move('left');
                    else
                        Lampa.Controller.toggle('menu');
    ...
    function startPlugin() {
        ...
        function FreeJaketOpt() {
            ...
            if (params) {
                ...
                if (param)
                    Lampa.Arrays.remove(params, param);
            }
                ...
                onRender: function(item) {
                    setTimeout(function() {
                        ...
                        if (Lampa.Storage.field('parser_use')) {
                            item.show();
                            if (Boolean(Modss.jack[Lampa.Storage.get('jackett_url2')]))
                                $('.settings-param__name', item).before('<div class="settings-param__status one ' + (Modss.jack[Lampa.Storage.get('jackett_url2')].ok ? "active" : "error") + '"></div>');
        ...
        Lampa.Settings.listener.follow('open', function(e) {
            ...
            if (e.name == 'mods_proxy') {
                ...
                $('.settings__title').text(Lampa.Lang.translate('menu_settings'));
            if (e.name == 'fork_param')
                $('.settings__title').append(" ForkTV");
        ...
        function add() {
            ...
            Lampa.SettingsApi.addParam({
                ...
                onRender: function(item) {
                    ...
                    item.on('hover:enter', function() {
                        if (Lampa.Storage.get('mods_password'))
                            Lampa.Input.edit({
                                ...
                                if (t == Lampa.Storage.get('mods_password'))
                                    pass();
                                else
                                    Lampa.Noty.show('Неверный пароль');
                            });
                    ...
                    if (Lampa.Storage.get('mods_password'))
                        ...
                        item.find('.settings-param__descr').text(Lampa.Lang.translate('placeholder_password'));
                },
                onChange: function(value) {
                    if (value)
                        $('body').find('.settings-param__descr').text('Изменить пароль');
                    else
                        $('body').find('.settings-param__descr').text(Lampa.Lang.translate('placeholder_password'));
                }
            });
            //Add-ons
            Lampa.SettingsApi.addParam({
            ...
            Lampa.SettingsApi.addParam({
                ...
                onRender: function(item) {
                    if (!Lampa.Storage.get('logined_pub'))
                        item.hide();
                    item.on('hover:enter', function() {
                        Pub.info_device();
                    });
            ...
            Lampa.SettingsApi.addParam({
                ...
                onRender: function(item) {
                    if (!Lampa.Storage.field('mods_tv'))
                        item.hide();
                    item.on('hover:enter', function() {
            ...
            Lampa.SettingsApi.addParam({
                ...
                onChange: function(value) {
                    if (value == 'true')
                        ...
                        $('body').find('.menu [data-action="collection"]').remove();
                }
            });
            //Styles
            Lampa.SettingsApi.addParam({
            ...
            Lampa.SettingsApi.addParam({
                ...
                onChange: function(value) {
                    if (value == 'true') {
                        ...
                        $('body').find('.rate--kp, .rate--imdb').addClass('hide');
                }
            });
            Lampa.SettingsApi.addParam({
                component: 'settings_modss',
                ...
                onChange: function(value) {
                    if (value == 'true' && $('body').find('.full-start__poster').length)
                        Modss.serialInfo(cards);
                    else
                        $('body').find('.files__left .time-line, .card--last_view, .card--new_seria').remove();
                }
            });
            if (/iPhone|iPad|iPod|android|x11/i.test(navigator.userAgent) || (Lampa.Platform.is('android') && window.innerHeight < 1080)) {
                ...
                Lampa.SettingsApi.addParam({
                    ...
                    onRender: function(item) {
                        if (Lampa.Storage.field('mods_butt_back'))
                            item.show();
                        else
                            item.hide();
                    },
        ...
        function url$1(u) {
            var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
            if (params.genres)
                u = add$4(u, 'genre=' + params.genres);
            if (params.page)
                u = add$4(u, 'page=' + params.page);
            if (params.query)
                u = add$4(u, 'q=' + params.query);
            if (params.type)
                u = add$4(u, 'type=' + params.type);
            if (params.field)
                u = add$4(u, 'field=' + params.field);
            if (params.id)
                u = add$4(u, 'actor=' + params.id);
            if (params.perpage)
                u = add$4(u, 'perpage=' + params.perpage);
        ...
        function main$2(params, oncomplite, onerror) {
            ...
            status.onComplite = function() {
                ...
                for (var i = 1; i <= 9; i++) {
                    var ipx = 's' + i;
                    if (data[ipx] && data[ipx].results.length)
                        fulldata.push(data[ipx]);
                }
                if (fulldata.length)
                    oncomplite(fulldata);
                else
        ...
        function url$2(u) {
            ...
            if (params.page)
                u = add$5(u, 'page=' + params.page);
            if (params.query)
                u = add$5(u, 'story=' + params.query);
            if (params.type)
                u = add$5(u, 'type=' + params.type);
            if (params.field)
                u = add$5(u, 'field=' + params.field);
            if (params.perpage)
                u = add$5(u, 'perpage=' + params.perpage);
        ...
        function search$4() {
            ...
            get$7('search', params, function(json) {
                ...
                if (json) {
                    json.forEach(function(element) {
                        if (element, element.last_episode && element.last_episode.season || element.serial_stats && element.serial_stats.status)
                            itemss.push(tocardf(element, element.last_episode && element.last_episode.season || element.serial_stats && element.serial_stats.status ? 'tv' : 'movie'));
                        else
                            items.push(tocardf(element, element.last_episode && element.last_episode.season || element.serial_stats && element.serial_stats.status ? 'tv' : 'movie'));
                    });
                    var movie = {
                        results: items,
                        page: 1,
